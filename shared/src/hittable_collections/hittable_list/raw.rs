mod type_shit {
    ...
    pub struct Functions {
        ...
        pub(crate) advance_by_n_shim: unsafe fn(*const u8, usize) -> *const u8,
        ...
        pub(crate) sort_by_axis: fn(*mut u8, usize, Axis),
    }
        ...
        T: Debug + BoundedHittable + Sync + Send + 'static,
    {
        const FUNCTIONS: Functions = Functions {
            ...
            sort_by_axis: |ptr, len, axis| {
                let vec = unsafe { slice::from_raw_parts_mut(ptr.cast::<T>(), len) };
                vec.sort_by(move |a, b| {
                    a.get_aabbox()
                        .axis(axis)
                        .start()
                        .total_cmp(b.get_aabbox().axis(axis).start())
                });
            },
        };
    }
...
use self::{
    ...
    debug_iterator::RawVecDebugIterator,
    hittable_iterator::RawVecHittableIterator,
    type_shit::{Functions, RawHittableVecFns},
};
...
impl RawHittableVec {
    ...
    pub const fn iter_debug(&self) -> impl Iterator<Item = &'_ dyn Debug> + '_ {
        RawVecDebugIterator {
            ptr: self.ptr,
            len: self.len,
            fns: self.fns,
    ...
    impl<'a> Iterator for RawHittableVecBoundedIterator<'a> {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            if self.len == 0 {
                ...
                let ptr = unsafe { (self.fns.advance_by_n_shim)(self.ptr, 1) };
                self.ptr = ptr;
                self.len -= 1;
        ...
        fn nth(&mut self, n: usize) -> Option<Self::Item> {
            if self.len <= n {
                ...
                let ptr = unsafe { (self.fns.advance_by_n_shim)(self.ptr, n) };
                let out_ptr = unsafe { (self.fns.into_bounded)(ptr) };
                let ptr = unsafe { (self.fns.advance_by_n_shim)(ptr, 1) };
                self.ptr = ptr;
                self.len -= n + 1;
                unsafe { out_ptr.as_ref() }
            }
...
mod hittable_iterator {
    ...
    pub struct RawVecHittableIterator<'a> {
        pub(crate) ptr: *const u8,
        pub(crate) len: usize,
        pub(crate) fns: &'static Functions,
        pub(crate) _phantom: PhantomData<&'a ()>,
    ...
    impl<'a> Iterator for RawVecHittableIterator<'a> {
        ...
        fn nth(&mut self, n: usize) -> Option<Self::Item> {
            if self.len <= n {
                ...
                let ptr = unsafe { (self.fns.advance_by_n_shim)(self.ptr, n) };
                let out_ptr = unsafe { (self.fns.into_hittable)(ptr) };
                let ptr = unsafe { (self.fns.advance_by_n_shim)(ptr, 1) };
                self.ptr = ptr;
                self.len -= n + 1;
                unsafe { out_ptr.as_ref() }
    ...
    impl ExactSizeIterator for RawVecHittableIterator<'_> {}
}
...
mod debug_iterator {
    ...
    pub struct RawVecDebugIterator<'a> {
        pub(crate) ptr: *const u8,
        pub(crate) len: usize,
        pub(crate) fns: &'static Functions,
        pub(crate) _phantom: PhantomData<&'a ()>,
    ...
    impl<'a> Iterator for RawVecDebugIterator<'a> {
        type Item = &'a dyn Debug;

        fn next(&mut self) -> Option<Self::Item> {
            if self.len == 0 {
                None
            } else {
                let out_ptr = unsafe { (self.fns.into_debug)(self.ptr) };
                let ptr = unsafe { (self.fns.advance_by_n_shim)(self.ptr, 1) };
                self.ptr = ptr;
                self.len -= 1;
                unsafe { out_ptr.as_ref() }
        ...
        fn nth(&mut self, n: usize) -> Option<Self::Item> {
            if self.len <= n {
                ...
                let ptr = unsafe { (self.fns.advance_by_n_shim)(self.ptr, n) };
                let out_ptr = unsafe { (self.fns.into_debug)(ptr) };
                let ptr = unsafe { (self.fns.advance_by_n_shim)(ptr, 1) };
                self.ptr = ptr;
                self.len -= n + 1;
                unsafe { out_ptr.as_ref() }
    ...
    impl ExactSizeIterator for RawVecDebugIterator<'_> {}
}
